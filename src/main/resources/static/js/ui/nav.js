...
export function renderNav(active, role = 'admin') {
    ...
    return `
    <nav class="flex md:flex-col items-center md:items-stretch gap-2.5">
      ${visibleItems.map((item) => {
        const activeClass = item.key === active ? 'bg-slate-900 text-white border-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100 border-transparent';
        const activeLabelClass = item.key === active ? 'text-white' : 'text-slate-700';
        return `<a href="#/${item.key}" title="${item.label}" class="h-14 w-14 rounded-xl md:rounded-lg md:w-full shrink-0 border flex items-center justify-center md:justify-start md:px-4 gap-3 ${activeClass}">
